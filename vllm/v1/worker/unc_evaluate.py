...
class SparseProbs:
    ...
    def from_dense_logits_top_p(cls, logits, p=0.9):
        ...
        for i in range(seq_len):
            ...
            while True:
                if k > vocab_size:
                    k = vocab_size
                logits_values_i, logits_indices_i = torch.topk(
                    sampled_logits_i, k=k, dim=-1)  # shape: [n_samples, seq_len, k]
                if torch.sum(logits_values_i) / sum_logits_i >= p:
                    ...
                    logits_remain_mean_i = (sampled_logits_i.sum(
                        dim=-1, keepdims=True) - logits_values_i.sum(dim=-1, keepdims=True))
                    remaining_vocab_sizes.append(torch.full_like(
                        logits_remain_mean_i, vocab_size - k))
                    ...
                    logits_values_i = torch.cat(
                        [logits_values_i, logits_remain_mean_i], dim=-1)
                    logits_indices_i = torch.cat([logits_indices_i, torch.full_like(
                        logits_indices_i[:, :1], vocab_size, dtype=logits_indices_i.dtype)], dim=-1)
        ...
        for seq_idx, (values_i, indices_i) in enumerate(zip(logits_values, logits_indices)):
            ...
            for batch_idx in range(n_samples):
                ...
                # Add batch indices
                all_batch_indices.append(torch.full(
                    (num_values,), batch_idx, device=device))

                # Add sequence indices
                all_seq_indices.append(torch.full(
                    (num_values,), seq_idx, device=device))
    ...
    def total_uncertainty(self):
        ...
        #    for each sample.
        compensate = torch.log(self.remaining_vocab_sizes.mean(
            dim=0)) * self.remaining_probs.mean(dim=0)
    ...
    def aleatoric_uncertainty(self):
        ...
        entropy = torch.cat([self.entropy(prob, log_prob).unsqueeze(
            0) for prob, log_prob in zip(probs, log_probs)], dim=0).mean(dim=0)

        # p * log(K) compensation for the under-estimation of the uncertainty.
        # NOTE: this is still under-estimation, but it's better than nothing.
        #    the two compensates are the same, as we have the same vocab size
        #    for each sample.
        compensate = (torch.log(self.remaining_vocab_sizes)
                      * self.remaining_probs).mean(dim=0)
        return entropy + compensate
...
class SparseLogits:
    ...
    def from_dense_tensor(cls, logits, k=200):
        ...
        # Here we add another slot (k+1) for storing the remaining logits value.
        # We don't care about the actual indices
        logits_values, logits_indices = torch.topk(
            logits, k=k, dim=-1)  # shape: [n_samples, seq_len, k]
        ...
        logits_remain_mean = (logits.sum(dim=-1) -
                              logits_values.sum(dim=-1)) / (vocab_size - k)
        ...
        logits_values = torch.cat([logits_values, math.log(
            vocab_size - k) + logits_remain_mean.unsqueeze(-1)], dim=-1)
        logits_indices = torch.cat([logits_indices, torch.full_like(
            logits_indices[:, :, :1], vocab_size, dtype=logits_indices.dtype)], dim=-1)

        # Create batch indices and sequence position indices
        batch_indices = torch.arange(
            n_samples).view(-1, 1, 1).expand(-1, seq_len, k+1).to(device)
        seq_indices = torch.arange(seq_len).view(
            1, -1, 1).expand(n_samples, -1, k+1).to(device)
        ...
        for i in range(seq_len):
            ...
            while True:
                ...
                if torch.sum(logits_values_i) / sum_logits_i >= p:
                    ...
                    # TODO: better way to estimate.
                    logits_remain_mean_i = (sampled_logits_i.sum(
                        dim=-1) - logits_values_i.sum(dim=-1)) / (vocab_size - k)
                    ...
                    # now the shape of logits_values is [n_samples, seq_len, vocab_size+1]
                    # and it can be directly used for uncertainty estimation.
                    logits_values_i = torch.cat([logits_values_i, math.log(
                        vocab_size - k) + logits_remain_mean_i.unsqueeze(-1)], dim=-1)
                    logits_indices_i = torch.cat([logits_indices_i, torch.full_like(
                        logits_indices_i[:, :1], vocab_size, dtype=logits_indices_i.dtype)], dim=-1)
                    logits_values.append(logits_values_i)
                    logits_indices.append(logits_indices_i)
...
def total_uncertainty(logits):
    ...
    """
    mean_logits = torch.log(
        F.softmax(logits, dim=-1).mean(dim=0))  # shape: [seq_len, vocab_size]
...
def evaluate_uncertainty_all_mem_efficient(logits, token_ids, mode="default"):
    ...
    for i in range(probs.size(1)):
        ...
        log_probs_buffer.nan_to_num_(0.0)  # replace NaN with 0
        ...
        log_probs_buffer[0].nan_to_num_(0.0)  # replace NaN with 0
    ...
    ##########################################
    tensor_token_ids = torch.tensor(
        token_ids, device=probs.device).squeeze(-1)  # shape: [seq_len,]
    ...
    if mode == "default":
        ...
        raise ValueError(
            f"Unknown mode for Bayesian Implicit Reward Estimation: {mode}.")
    ...
    birs = [0. for _ in range(len(tus) - r_lose.shape[0])] + \
        (r_win - r_lose).cpu().tolist()
